!Issue number DPD200237442
!!
!! ILLEGAL AMBIGUOUS TYPE BOUND GENERICS NOT DETECTED 
!!
!! The Intel 13.0.0 compiler does not detect when illegal ambiguous type bound
!! generic functions are defined.  The following example defines a type FOO
!! with type-bound generic name, assignment, and operator.  An extended type
!! BAR adds to these generics, but the pair of specific bindings for each of
!! the three generics are ambiguous according to the rules given in 12.4.3.4.5
!! (see also C.9.6 par 3).
!!
!! % ifort --version
!! ifort (IFORT) 13.0.0 20120731
!! 
!! % ifort -c intel-bug-20121013.f90 
!! (no errors reported)
!!
